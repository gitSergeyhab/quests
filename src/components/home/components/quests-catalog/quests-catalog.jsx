...
import { setDisplayQuests, setGenre } from 'store/actions';
import { getDisplayQuests, getGenre, getQuestsLoadedStatus } from 'store/catalog-reducer/catalog-reducer-selectors';
...
const QuestItem = ({quest}) => {
  ...
  const displayCount = peopleCount && peopleCount.length ? `${peopleCount[0]} - ${peopleCount[peopleCount.length - 1]} чел` : '...';
...
const QuestsCatalog = () => {
  ...
  const questions = useSelector(getDisplayQuests);
  const areQuestsLoaded = useSelector(getQuestsLoadedStatus);
  const genre = useSelector(getGenre);


  useEffect(() => {
    console.log('useEffect', genre, areQuestsLoaded, questions)
    dispatch(setDisplayQuests())
